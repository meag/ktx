...
void xml_teams_footer(fileHandle_t handle)
{
	s2di(handle, "\t</teams>\n");
}
...
static void xml_player_header(fileHandle_t handle, gedict_t* player, char* team)
{
...
void xml_player_detail(fileHandle_t handle, int num, gedict_t* player)
{
	int j;

	xml_player_header(handle, player, getteam(player));
